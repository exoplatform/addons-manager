...
import org.exoplatform.platform.am.utils.FileEncrypt
 ...
 */
public class AddonApplyService {
  /**
   * Logger
   */
   ...
   * Singleton
   */
  private static final AddonApplyService singleton = new AddonApplyService()

  /**
   * Factory
   *
   * @return The {@link AddonApplyService} singleton instance
   */
  static AddonApplyService getInstance() {
    return singleton
  }

  private long getFileCRC(File file) throws IOException {
  ...
  private Object generateAddonCatalog(File patchfile){
    String patchfilename = patchfile.getName()
    String patchid = patchfilename.replaceAll("-\\d.zip", "")
    String patchversion = patchfilename.replaceAll("patch-\\d+(\\.\\d+)*-", "").replace(".zip", "")
    String plfversion = patchfilename.replaceAll("-\\d.zip", "").replace("patch-","")
    return [
            "id":patchid,
            "version": patchversion,
            "unstable": false,
            "name": "Patches for eXo Platform " + plfversion,
            "description": "Patches for eXo Platform " + plfversion,
            "releaseDate": "2018-08-09T22:00:00.000Z",
            "downloadUrl": "file://" + patchfile.getAbsolutePath(),
            "vendor": "eXo",
            "author": "eXo Support",
            "authorEmail": "support@exoplatform.com",
            "license": "LGPLv3",
            "licenseUrl": "https://www.gnu.org/licenses/lgpl-3.0.txt",
            "mustAcceptLicense": false,
            "supportedDistributions": "enterprise",
            "supportedApplicationServers": "tomcat",
            "compatibility": plfversion
    ]
  }
  ...
  void apply (
          ...
          CommandLineParameters.ApplyCommandParameters parameters,
          Boolean batchMode) {
    Pattern p = Pattern.compile("patch-\\d+(\\.\\d+)+-\\d+.zip.enc")
    Matcher m = p.matcher(parameters.patchFile)
    if (!m.find()) {
      LOG.error("Wrong eXo patch package ${parameters.patchFile} !")
      return
    }
    p = Pattern.compile("\\w+.exokey\$")
    m = p.matcher(parameters.keyFile)
    if (!m.find()) {
      LOG.error("Wrong eXo Key File ${parameters.keyFile} !")
      return
    }
    File encryptedFile = new File(parameters.patchFile)
    if(!encryptedFile.exists())
    {
      LOG.error("Could not find ${encryptedFile.getName()} File.")
      return
    }
    File keyFile = new File(parameters.keyFile)
    if(!keyFile.exists())
    {
      LOG.error("Could not find ${keyFile.getName()} File.")
      return
    }
    LOG.withStatus(
            "Decrypting ${encryptedFile.getName()} File") {
      FileEncrypt fileEncrypt = new FileEncrypt();
      if (!fileEncrypt.decryptFile(keyFile, encryptedFile)) {
        throw new UnknownErrorException("Could not decrypt ${encryptedFile.getName()} File.")
      }
    }
    def decryptedFile = new File (encryptedFile.getAbsolutePath().replaceAll(".enc\$",""))
    // Check CRC
      if( parameters.crcCheck != 0L) {
        long givenCrc = parameters.crcCheck
		  LOG.withStatus(
              "Checking CRC of ${decryptedFile.getName()} File") {
	      if (givenCrc != getFileCRC(decryptedFile)) {
          throw new UnknownErrorException("${decryptedFile.getName()} File is tampered!.")
        }
      }
   }
    def objAddon = generateAddonCatalog(decryptedFile)
    AddonService.ParsingErrors errors
    Addon addon = ADDON_SERVICE.createAddonFromJsonObject(objAddon,errors)
    apply(env,addon,parameters.force,parameters.noCompat,parameters.conflict,batchMode)
  ...
  /**
   * Install the @{code addon} in      parameters.addonId,
to the current @{code env}* @param env The execution environment
 ...
 protected void apply(
    ...
    // Let's download it
   if (addon.downloadUrl.startsWith("file://")) {
